...
- 标题与粗体颜色可以单独选择跟随当前版式或 Obsidian 主题；正文颜色仍由纸张主题决定。
- “推荐写作版式”提供适合中文长文的起点。
...
- 也可以导入 `.ttf`、`.otf`、`.woff` 或 `.woff2` 字体文件。导入后会立即选中并应用，适合需要桌面端与移动端保持一致的情况。字体文件保存在 Obsidian 配置目录下独立的 `chinese-writing-layout/fonts/` 中，避免插件更新时替换用户文件；`data.json` 只保存引用信息。
...
现有规则覆盖空白、空行、段首、标点、引号、章节标题和 Markdown 标记等常见问题。可以在章节标题规则中选择 `第12章`、`第十二章`、`（十二）` 或 `12、` 格式，也可以使用内置方案、保存自定义方案、调整规则顺序，按手动选择、文件夹或 Tag 批量处理。YAML/frontmatter、围栏代码块、行内代码、评论、数学、脚注和表格等结构会受到保护。
...
- **高亮当前行**：只在显示层提示当前输入行。
- **正文导航**：在长文编辑器中提供滚动到顶部 / 底部的悬浮按钮，可在设置中分别关闭。
...
- 用户导入的字体文件保存在 Obsidian 配置目录下独立的 `chinese-writing-layout/fonts/` 子目录，不会被插件更新覆盖。旧版插件目录中的字体会在启动时安全迁移；迁移失败时仍会从旧位置加载。
